...
use dotenv::{lines_to_entries, lines_to_json, Line};
...
pub async fn encrypt_lines_to_sec(
    ...
    let old_value_to_id: HashMap<String, String> = match load_existing_secrets(sec_file, encryption_engine).await {
        Ok(old_secrets) => {
            old_secrets.into_iter().map(|(id, value)| (value, id)).collect()
        }
        Err(_) => HashMap::new(),
    };
        ...
        match line {
            ...
            Line::Kv(key, value, quote_type) => {
                ...
                if let Some(real_value) = secrets.get(value) {
                    resolved.push(Line::Kv(key.clone(), real_value.clone(), quote_type.clone()));
                } else {
...
/// Returns the {id: real_value} HashMap, or an error if the file doesn't exist or can't be decrypted.
async fn load_existing_secrets(
    ...
    let content = load_file(sec_file)?;
    let lines = dotenv::parse_dotenv(&content)?;
    let dotsec_value = dotenv::get_value(&lines, "__DOTSEC__")
...
fn lookup(name: &str, resolved: &[(String, String)]) -> String {
    // Check our resolved vars first (last wins for duplicates)
...
mod tests {
    ...
    fn resolve_env_vars_interpolation() {
        let lines = vec![
            ...
            Line::Newline,
            Line::Kv("URL".into(), "http://${HOST}:3000".into(), QuoteType::Double),
        ];
        let resolved = resolve_env_vars(&lines);
    ...
    fn redact_replaces_secrets() {
        ...
        assert_eq!(redact("my password is s3cret", &secrets), "my password is ******");
    }

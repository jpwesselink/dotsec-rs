...
impl Schema {
    /// Create a new Schema from an IndexMap.
    pub fn new(entries: IndexMap<String, SchemaEntry>) -> Self {
        Self { entries }
    }
    ...
    /// Insert a schema entry. If the key already exists, it is replaced.
    pub fn insert(&mut self, entry: SchemaEntry) {
        self.entries.insert(entry.key.clone(), entry);
    }

    /// Extend with additional entries.
    pub fn extend(&mut self, entries: impl IntoIterator<Item = SchemaEntry>) {

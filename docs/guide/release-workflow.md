...
Versioning is automated through [conventional commits](https://www.conventionalcommits.org/) and [release-plz](https://release-plz.ieni.dev/). Distribution is **npm-only** — see [why](#why-no-cratesio) below.
...
|---|---|---|
| **CI** | `ci.yml` | Runs on every push and PR. Gates: `cargo fmt --all -- --check`, `cargo build --workspace --all-targets`, `cargo test --workspace --all-targets`, `cargo clippy --workspace --all-targets -- -D warnings`, and a separate `cargo audit` job (with `.cargo/audit.toml` ignores for unactionable transitive advisories) |

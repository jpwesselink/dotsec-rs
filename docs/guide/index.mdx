...
<Tabs>
  ...
  <Tab label="npm (dev dep)">
    ...
    # or: pnpm add -D dotsec / yarn add -D dotsec
    ```
    ...
    See [Setup → Install](/guide/setup#install) for the full pattern.
  </Tab>
...
> Distribution is npm-only. The obvious crate names on crates.io (`dotsec`, `crypto`, `dotenv`, `aws`) are owned by unrelated projects, so `cargo install` isn't available. The `dotsec` npm package bundles a per-platform native binary.
...
All distribution is npm-only — see the note in [Install](#install) above.

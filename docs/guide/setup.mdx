...
`.sec.key` is auto-added to `.gitignore` the first time `dotsec set` or `dotsec init` generates a keypair: if no `.gitignore` exists in a git repo, one is created with `*.key`; if it already exists without that pattern, `*.key` is appended. Pass `--no-gitignore` to opt out (you take responsibility for excluding the file then). Nothing happens outside a git repo.
